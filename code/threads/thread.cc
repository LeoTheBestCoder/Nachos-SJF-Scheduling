...
Thread::Sleep (bool finishing)
{
    ...
    int ApproximateBurstTime = 0.5 * kernel->currentThread->getRunTime() 
                             + 0.5 * kernel->currentThread->getPredictedBurstTime();
    ...
    DEBUG(dbgSJF, "[S] Tick [" << kernel->stats->totalTicks << "] : Thread [" << nextThread->getID()
                   << "] is now selected for execution, thread [" << kernel->currentThread->getID()
                   << "] is replaced, and it has executed for [" << kernel->currentThread->getRunTime()
                   << "] ticks");
    ...
    if (kernel->currentThread->getID() > 0) {
        DEBUG(dbgSJF, "[U] Tick [" << kernel->stats->totalTicks << "]: Thread [" 
                       ...
                       << PrevBurstTime << "], add [" 
                       << kernel->currentThread->getPredictedBurstTime() - PrevBurstTime 
