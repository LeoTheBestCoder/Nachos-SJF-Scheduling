...
Scheduler::ReadyToRun (Thread *thread)
{
    ...
    if (readyQueue->IsEmpty() != 1) {
        DEBUG(dbgSJF, "***Thread [" << readyQueue->Front()->getID() << "]'s and thread [" << thread->getID()
                    << "]'s burst time are " << readyQueue->Front()->getPredictedBurstTime() << " and "
                    <<  thread->getPredictedBurstTime()<< "***");
    }

	thread->setStatus(READY);
	// readyList->Append(thread);
    DEBUG(dbgSJF, "[I] Tick [" << kernel->stats->totalTicks << "]: Thread [" << thread->getID() 
                   << "] is inserted into the readyQueue");
...
Scheduler::FindNextToRun ()
{
	...
	if (readyQueue->IsEmpty()) {
        ...
        DEBUG(dbgSJF, "[R] Tick [" << kernel->stats->totalTicks << "]: Thread [" 
                       << temp->getID() << "] is removed from the readyQueue");
		return temp;
        // </add>
...
static int SortingRule(Thread *t1, Thread *t2) {
    ...
    //     return -1;
    // else if (t1BurstTime == t2BurstTime && t1id > t2id)
    //     return 0;
    // else if (t1BurstTime == t2BurstTime && t1id <= t2id)
    //     return 1;
    // else
